...
class LinesFormatter {
  ...
  async security(line) {
    let cleanedLine
    try {
      cleanedLine = await this.linesUtility.prepareSecurity(line)
    } catch (error) {
      throw error
    }
    ...
    try {
      ...
      return [segments[0], price, segments[2], segments[3]]
    } catch (error) {
      throw error
    }
  }
   ...
   * @return {!Array<string>} Array of identified values.
   */
  async investment(line) {
    try {
      return await this.securityDeposit(line)
    } catch (error) {
      throw error
    }
  }
