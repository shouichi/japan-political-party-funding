...
class LinesUtility {
  ...
  async prepareSpace(line) {
    // if it's already prepared, simply just return it
  ...
  async prepareMovableProperty(line) {
    ...
    let name = null
    let price = null
    const rest = dates.rest.trim()
    for (let index = rest.length - 1; index >= 0; index--) {
      const character = rest.charAt(index)
      if (!character.match(/\d|,/)) {
        name = rest.substr(0, index + 1).trim()
        price = rest.substr(index + 1).trim()
        break
      }
    }

    return `${name} ${price} ${dates.era} ${dates.date}`
  }
  ...
  async extractTailingDate(line) {
    ...
    return { date: formattedDate, era: era, rest: rest }
  }
}
