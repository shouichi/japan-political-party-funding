...
class LinesUtility {
  ...
  async prepareOneNameAndOnePrice(line) {
    let nameAndPrice
    try {
      nameAndPrice = await this.extractTailingNameAndPrice(line)
    } catch (error) {
      throw error
    }
   ...
   * @return {string} Line prepared.
   */
  async prepareLending(line) {
    try {
      return await this.prepareDebt(line)
    } catch (error) {
      throw error
    }
  }
   ...
   * @return {string} Line prepared.
   */
  async prepareInvestment(line) {
    try {
      return await this.prepareSecurity(line)
    } catch (error) {
      throw error
    }
  }
