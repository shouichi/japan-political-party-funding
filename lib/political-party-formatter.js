...
const ASSET_SECURITY = '有価証券'
const ASSET_LENDING = '貸付金'
const ASSET_INVESTMENT = '出資による権利'
const ASSET_FACILITY_USE = '施設の利用に関する権利'
const ASSET_BREAKDOWN_CATEGORIES = [
  ...
  { name: ASSET_SECURITY, regex: /^\s*〔有価証券〕/ },
  { name: ASSET_LENDING, regex: /^\s*〔貸付金〕/ },
  { name: ASSET_INVESTMENT, regex: /^\s*〔出資による権利〕/ },
  { name: ASSET_FACILITY_USE, regex: /^\s*〔施設の利用に関する権利〕/ },
]
...
class PoliticalPartyFormatter {
  ...
  /**
   * Formats a political party section of the text.
   * @return {!Map} Resulting Map object.
   */
  async format() {
  ...
  async formatDonationBreakdown(categories) {
    ...
    for (const donationBreakdownCategory of donationBreakdownCategories) {
      ...
      donationBreakdown.set(categoryName, data)
    }
    ...
    for (const assetBreakdownCategory of assetBreakdownCategories) {
      ...
      for (const line of collected) {
        if ([ASSET_LAND, ASSET_BUILDING].includes(categoryName)) {
          ...
          data.push(formatted)
        } else if ([ASSET_SECURITY].includes(categoryName)) {
          const formatted = await this.linesFormatter.security(line)
          data.push(formatted)
        } else if ([ASSET_LENDING].includes(categoryName)) {
          const formatted = await this.linesFormatter.lending(line)
          data.push(formatted)
        } else if ([ASSET_INVESTMENT].includes(categoryName)) {
          const formatted = await this.linesFormatter.investment(line)
          data.push(formatted)
        } else if ([ASSET_FACILITY_USE].includes(categoryName)) {
          const formatted = await this.linesFormatter.facilityUse(line)
          data.push(formatted)
        } else {
          data.push(line)
        }
